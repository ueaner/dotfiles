...
async def _detect_active_adapter_proc_names(adapters: list[type[Compositor]]) -> set[str]:
    ...
    if process.returncode == 0:
        ...
        return {parts[1] for line in output.splitlines() if (parts := line.split(maxsplit=1)) and len(parts) > 1}
...
async def detect() -> Compositor:
    ...
    xdg_desktop = {n.strip().lower() for n in os.environ.get("XDG_CURRENT_DESKTOP", "").split(":") if n.strip()}
    ...
    if waylaunch:
        for adapter_cls in adapters:
            ...
            if any(name.lower() == waylaunch for name in meta.desktop_names):
                return adapter_cls()
        logger.warning(f"Specified compositor '{waylaunch}' not found, falling back to auto-detection.")

    # 2. 自动探测环境
    for adapter_cls in adapters:
        meta: DiscoveryMeta = adapter_cls.metadata()
